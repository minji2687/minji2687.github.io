    ...
    <Container className="py-16">
      <div className="max-w-2xl">
        ...
        <div className="mt-8 space-y-6 text-base leading-8 text-[var(--muted)]">
          <p>
            안녕하세요, 5년차 프론트엔드 개발자 조민지입니다. 최근 3년은
            농기계 자율주행 플랫폼에서 실시간 데이터 기반의 제어·모니터링
            시스템을 담당했고, Next.js 웹 서비스와 React Native 모바일
            앱을 함께 다뤘습니다. 하드웨어 단말기에서 발생한 데이터가
            사용자 화면까지 안정적으로 전달·표현되도록 프론트엔드 구조를
            구현하고 개선하는 데 집중했습니다.
          </p>
          <p>
            AWS IoT Core, MQTT, WebSocket 기반으로 수신한 장비 데이터를
            처리하며, CAN 버스 바이너리 데이터를 파싱해 실시간 UI 상태와
            동기화하고, 장비 상태·위치 정보·작업 이력이 화면에 안정적으로
            반영되도록 구현했습니다. MapLibre 기반 위치·경로 시각화도
            함께 구축했습니다.
          </p>
          <p>
            복잡한 데이터 흐름을 신뢰할 수 있는 화면 경험으로 연결하는 일에
            관심이 많고, 통신 지연이나 상태 불일치처럼 운영 환경에서 발생하는
            문제를 차분히 들여다보는 과정을 좋아합니다. 대학에서 서양화를
            전공한 경험은 지도 위에 데이터를 어떻게 보여줄지 같은 시각화
            작업에서 여전히 도움이 되는 감각으로 남아있습니다.
          </p>
        </div>
