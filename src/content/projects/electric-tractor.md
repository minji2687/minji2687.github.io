...
제어 명령 송신과 CAN 데이터 수신은 AWS API Gateway 기반 WebSocket으로 처리한다. 이 부분은 이전 개발자가 만들어둔 구조를 이어받은 것이었다.

문제는 이 앱이 차량과 실제로 연결되어 있는지, 즉 "살아있는지"를 확인하는 부분이었다. 기존 WebSocket 쪽에는 이 상태를 판단할 만한 코드를 찾을 수 없었다. 그래서 별도로 AWS IoT Core에 `react-native-paho-mqtt`로 MQTT 연결을 새로 맺고, `alive-signal` 토픽 하나만 구독해서 연결 상태를 확인하는 채널을 추가했다. 결과적으로 통신 채널이 두 개로 나뉘게 됐다 — 제어/데이터는 기존 WebSocket, 연결 상태 확인은 새로 추가한 MQTT.
...
alive-signal이 5초 안에 다시 오지 않으면 `mqttConnected`를 `false`로 떨어뜨린다. 연결이 살아있다는 걸 한 번 확인하는 것과, 끊겼다는 걸 빠르게 감지하는 것은 다른 문제라서 별도 타이머로 분리했다.
...
- [ ] WebSocket과 MQTT를 분리한 이유(왜 제어/CAN 데이터는 WebSocket으로 보냈는지) 좀 더 구체적으로 설명
